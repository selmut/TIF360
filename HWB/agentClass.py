    ...
    def fn_turn(self):
        if self.gameboard.gameover:
            ...
            if self.episode % 100 == 0:
                with open('data/rewards/r_avg.csv', 'a') as f:
                    f.write(f'{self.episode},{np.sum(self.reward_tots[range(self.episode-100, self.episode)])/100}\n')
...
class TDQNAgent:
    ...
    def fn_init(self, gameboard):
        ...
        self.Na = self.gameboard.N_col**2
...
class QConvNetwork(nn.Module):
    def __init__(self, output_size):
        ...
        self.linear1 = nn.Linear(in_features=72, out_features=128)
    ...
    def forward_old(self, x):
        ...
        if len(x.shape) == 3:
            tile_id = x[0, 0, :]
            board_state = x[0, 1:, :]
            board_state = board_state.reshape((1, 8, 8))

        else:
            tile_id = x[:, 0, 0, :]
            # tile_id = tile_id.reshape((32, 1, 8))

            board_state = x[:, 0, 1:, :]
            board_state = board_state.reshape((32, 1, 8, 8))

        board_state = self.conv1(board_state)
        board_state = self.relu(board_state)
        board_state = self.conv2(board_state)
        board_state = self.relu(board_state)
        board_state = self.max_pool1(board_state)

        board_state = self.flatten(board_state)

        out = [board_state, tile_id]
...
class TDConvQNAgent:
    ...
    def fn_init(self, gameboard):
        ...
        self.tile_identifiers = self.fn_get_tile_identifiers()
        self.action_map = self.idx_to_action()  # (tile, rot, col)
        ...
        self.model = QConvNetwork(self.Na).float()
        self.target_model = deepcopy(self.model)
    ...
    def fn_select_action(self):
        ...
        if rand < self.epsilonE:
            ...
            self.action_idx = sorted_qvals_idx[-1]

        col = self.action_idx // 8
        rot = self.action_idx % 4
    ...
    def fn_reinforce(self, batch):
        ...
        first_states = np.array([transition[0] for transition in batch])
        first_qvals_batch = self.model(torch.from_numpy(first_states.reshape((32, 1, 9, 8)))).detach().numpy()

        second_states = np.array([transition[3] for transition in batch])
        second_qvals_batch = self.target_model(torch.from_numpy(second_states.reshape((32, 1, 9, 8)))).detach().numpy()
        ...
        self.model.train()
        self.loss = self.criterion(self.model(torch.from_numpy(X.reshape((32, 1, 9, 8)))), torch.from_numpy(y))
