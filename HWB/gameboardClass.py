...
class TGameBoard:
    ...
    def __init__(self, N_row, N_col, tile_size, max_tile_count, agent, stochastic_prob):
        ...
        if self.tile_size == 2:
            ...
            # columns taken by the tile. The second dimension consist of pairs giving the y range: first element in
            # the pair is the first row of the tile and the second element second is the last row plus 1 of the tile
    ...
    def fn_new_tile(self):
        if self.tile_count < self.max_tile_count:
            # Choose a random tile with probability stochastic_prob, otherwise take tile from deterministic
            # sequence of tiles
