...
# If you change 'strategy_file' to the location of a file containing a stored Q-table or Q-network, you can evaluate
# the success of the found strategy
...
# 'stochastic_prob' (float between 0 and 1) denotes the probability to take a random tile. When stochastic_prob=0 tiles
# are taken according to a predefined sequence, when stochastic_prob=1 all tiles are random. For values
# 0<stochastic_prob<1 there is a mixture between deterministic and random tiles
...
# 'sync_target_episode_count' is the number of episodes between synchronisations of the target network
            ...
            if gameboard.cur_tile_type is not None:
                ...
                for xLoop in range(len(curTile)):
                    for yLoop in range(curTile[xLoop][0], curTile[xLoop][1]):
                        pygame.draw.rect(screen, COLOR_RED, [101+20*((xLoop+gameboard.tile_x) % gameboard.N_col),
                                                             81+20*(gameboard.N_row-(yLoop+gameboard.tile_y)), 18, 18])

            screen.blit(font.render("Reward: "+str(agent.reward_tots[agent.episode]), True, COLOR_BLACK), [0, 0])
            screen.blit(font.render("Tile "+str(gameboard.tile_count)+"/"+str(gameboard.max_tile_count), True,
                                    COLOR_BLACK), [0, 20])
            if framerate > 0:
